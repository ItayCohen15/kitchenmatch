    ...
    <div className="screen-enter space-y-4 pb-2">
      ...
      <div className="bg-gradient-to-l from-km-navy to-km-dark rounded-2xl p-5 text-white">
        ...
        <div className="grid grid-cols-3 gap-3">
          {[
            { label: '׳׳¨׳ ׳§', value: `ג‚×${walletBalance.toLocaleString()}`, icon: 'נ’³' },
            { label: '׳׳©׳׳¨׳•׳×', value: `${recentJobs.length}`, icon: 'נ“‹' },
            { label: '׳¢׳•׳‘׳“׳™׳ ׳–׳׳™׳ ׳™׳', value: `${workers.length}`, icon: 'נ‘·' },
          ].map(s => (
          ...
          <div className="flex items-center gap-2 mb-1">
            ...
            <span className="font-bold text-sm">
              {activeShift.Status === 'active' ? '׳׳©׳׳¨׳× ׳₪׳¢׳™׳׳” ׳¢׳›׳©׳™׳•' :
               activeShift.Status === 'confirmed' ? 'ג… ׳¢׳•׳‘׳“ ׳׳•׳©׳¨ ג€” ׳׳׳×׳™׳ ׳׳”׳’׳¢׳”' :
               'ג³ ׳׳׳×׳™׳ ׳׳׳™׳©׳•׳¨ ׳¡׳™׳•׳'}
            </span>
          </div>
          <div className="font-black text-lg">{activeShift.WorkerName || '׳¢׳•׳‘׳“'}</div>
          <div className="text-green-100 text-sm mb-3">
            {activeShift.Role} ֲ· ג‚×{activeShift.HourlyRate}/׳©׳³
          </div>
      ...
      {/* Quick actions */}
      <div>
        <h2 className="font-bold text-gray-800 mb-3 text-base">׳₪׳¢׳•׳׳•׳× ׳׳”׳™׳¨׳•׳×</h2>
        <div className="grid grid-cols-2 gap-3">
          ...
          >
            ...
            <div className="text-amber-100 text-xs mt-0.5">׳׳¦׳ ׳¢׳•׳‘׳“ ׳¢׳›׳©׳™׳•</div>
          </button>
          <button
            onClick={() => { navToRestaurant('create_job'); }}
          ...
          >
            <div className="flex items-center gap-1 mb-2">
              ...
              <span className="text-xs font-bold bg-white/20 px-2 py-0.5 rounded-full">׳—׳™׳¨׳•׳</span>
            </div>
            <div className="font-bold">׳׳¦׳‘ ׳—׳™׳¨׳•׳</div>
            <div className="text-red-100 text-xs mt-0.5">׳¢׳•׳‘׳“ ׳×׳•׳ 30 ׳“׳§׳•׳×</div>
          </button>
        </div>
      </div>
              ...
              <div key={w.Id} className="bg-white rounded-xl p-3 flex items-center gap-3 card-shadow">
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2">
                    ...
                    {w.Rating > 0 && <span className="text-xs text-yellow-500 font-bold">ג˜…{w.Rating.toFixed(1)}</span>}
                  </div>
                  <div className="text-gray-500 text-xs">{w.City} ֲ· {w.Role}</div>
                </div>
                <div className="text-right flex-shrink-0">
                  <div className="text-amber-500 font-bold text-sm">ג‚×{w.HourlyRate}/׳©׳³</div>
            ...
            {recentJobs.map((j: any) => (
              <div key={j.Id} className="bg-white rounded-xl p-3 flex items-center gap-3 card-shadow">
                ...
                <div className="flex-1">
                  ...
                  <span className="text-gray-400 text-xs"> ֲ· {j.Status === 'searching' ? '׳׳—׳₪׳©' : j.Status === 'completed' ? '׳”׳•׳©׳׳' : j.Status}</span>
                </div>
                <div className="text-right">
                  <div className="text-gray-700 font-bold text-sm">ג‚×{j.HourlyRate}/׳©׳³</div>
                  {j.TotalPay && <div className="text-green-600 text-xs">ג‚×{j.TotalPay} ׳¡׳”׳´׳›</div>}
                </div>
              </div>
            ))}
          </div>
